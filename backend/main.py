    ...
    try:
        ...
        try:
            ...
            return JSONResponse(
                ...
                content={
                    "error": (
                        "Unable to generate day-wise work using Gemini. "
                        "Please verify your API key, model access, and try again in a minute. "
                        f"Details: {e}"
                    )
                },
            )
...
@app.get("/")
async def root():
    return JSONResponse(content={"message": "OJT Journal Maker API is running."})
